// https://github.com/torinaki/fightcodegame_robot/blob/96f78ba7d0813f8e73e41e7e38502c19e0d29238/numberuno.js
// Dmitry Balabka on Apr 1, 2013 1673 sloc
...
(function () {
  ...
  a.onRobotCollision = function (a) {
    ...
    if (f > 0.3)c.move(1, -1); else if (f < -0.3)c.move(1, 1); else c.turn(1);
  };
  ...
  a.next = function (a) {
    try {
      ...
      if (b.tracker.isHunting(b))this.huntNext(a, b); else this.scanNext(a, b);
    } catch (c) {
    }
  };
  ...
  a.driveWithin = function (a, b) {
    ...
    if (Math.abs(d - e) > 0.2)if (e > d)a.move(1, 1); else a.move(1, -1); else a.turn(1);
    b.arcAndDirection = null;
  };
  ...
  b.prototype.nextOrtho = function (a) {
    ...
    if (c < 0)c += 360;
    var d = c % 90;
    if (Math.abs(d) > 1e-2)if (a < 0)return b.FromAngle(c - d); else return b.FromAngle(c + 90 - d);
  ...
  d.prototype.intersectsWith = function (a) {
    if (a instanceof d)if (this.direction.isParallelTo(a.direction))return false;
    if (a instanceof e)if (this.direction.isParallelTo(a.direction()))return false;
  ...
  e.prototype.intersectsWith = function (a) {
    ...
    if (a instanceof e)if (!f.isOnLine(a))return false;
    if (a instanceof d)if (!f.isOnRay(a))return false;
  ...
  g.prototype.getIntersectionPoints = function (a, b) {
    var c = [];
    var e = a;
    if (b)if (!a.startPoint.isWithin(this, 0))e = new d(this.trimToWithin(a.startPoint, 1), a.direction);
    ...
    if (c.length > 1)if (e.startPoint.distanceTo(c[1]) < e.startPoint.distanceTo(c[0])) {
      var i = c[0];
      c[0] = c[1];
      c[1] = i;
    }
  ...
  g.prototype.getClosestLines = function (a) {
    ...
    do {
      c = false;
      for (var d = 0; d < b.length - 1; d++)if (b[d].distanceToPoint(a) > b[d + 1].distanceToPoint(a)) {
        var e = b[d];
        b[d] = b[d + 1];
        b[d + 1] = e;
        c = true;
  ...
  i.prototype.push = function (a, b) {
    ...
    if (c != null)if (b.time < (c.time + 10))return;
    var d = new m(a, b.time);
    if (!d.isOpponent(b))return;
    if (d.isParent())this.parentScans.push(d); else if (d.isClone())this.cloneScans.push(d); else if (d.isUnknown())this.unknownScans.push(d);
  };
  ...
  i.prototype.lastTrackingScans = function (a, b) {
    ...
    if (a == null)if (this.parentScans.isTracking(b) && this.cloneScans.isTracking(b))c = this.parentScans.lastTime() < this.cloneScans.lastTime() ? this.cloneScans : this.parentScans; else if (this.parentScans.isTracking(b) && !this.cloneScans.isTracking(b))c = this.parentScans; else if (!this.parentScans.isTracking(b) && this.cloneScans.isTracking(b))c = this.cloneScans;
    return c;
  };
  ...
  o.prototype.isWallHugger = function (a) {
    ...
    if (e < a.arena.radius + 2)if (e < a.arena.radius + 2) {
  ...
  q.prototype.getArcRadius = function (a, b, c) {
    ...
    while (m < Math.max(b, Math.abs(h))) {
      ...
      if (A > b && C < 0)if (n == null)n = new r(A, C); else {
        var D = A - b;
        var E = n.timeOffset - b;
        if (D < E)n = new r(A, C);
      }
      if (z > b && B > 0)if (n == null)n = new r(z, B); else {
        var F = z - b;
        var E = n.timeOffset - b;
        if (F < E)n = new r(z, B);
      }
  ...
  s.prototype.angleDiff = function (a, b) {
    var c = b - a;
    if (Math.abs(c) < 180)return c; else {
